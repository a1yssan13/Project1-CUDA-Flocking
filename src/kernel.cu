...
void Boids::initSimulation(int N) {
  ...
  cudaMalloc((void**)&dev_pos_re, N * sizeof(glm::vec3));
  checkCUDAErrorWithLine("cudaMalloc dev_pos_re failed!");

  cudaMalloc((void**)&dev_vel_re, N * sizeof(glm::vec3));
  checkCUDAErrorWithLine("cudaMalloc dev_vel_re failed!");
...
__device__ glm::vec3 computeVelocityChange(int N, int iSelf, const glm::vec3 *pos, const glm::vec3 *vel) {
    ...
    for (int j = 0; j < N; j++) {
        ...
        glm::vec3 thatBoidVelocity = vel[j]; 
        float distance = glm::length(thatBoidPos - thisBoid);
        if (distance < rule1Distance) {
            perceivedCenter += thatBoidPos;
            neighborCount1++;
        }
        if (distance < rule2Distance) {
            c -= (thatBoidPos - thisBoid);
        }
        if (distance < rule3Distance) {
            perceivedVelocity += thatBoidVelocity;
            neighborCount3++;
        }
    }
    ...
    if (neighborCount1 > 0) {
        ...
        newVelocity += (perceivedCenter - thisBoid) * rule1Scale;
    }
...
__global__ void kernUpdateVelocityBruteForce(int N, glm::vec3 *pos,
  ...
  // Compute a new velocity based on pos and vel1
   ...
   // Clamp the speed
   if (speed > maxSpeed) {
       // Normalize the velocity vector and scale it to maxSpeed
...
__global__ void kernIdentifyCellStartEnd(int N, int *particleGridIndices,
  ...
  // "this index doesn't match the one before it, must be a new cell!"
    ...
    int prevIndex = particleGridIndices[imax(0, index - 1)]; //Grid Index of previous array 
    int nextIndex = particleGridIndices[imin(index + 1, N - 1)];
    ...
    if (currIndex != nextIndex || currIndex == N - 1) {
        gridCellEndIndices[currIndex] = index + 1;
    }
}
  ...
  for (int i = minGrid.x; i <= maxGrid.x; i++) {
      for (int j = minGrid.y; j <= maxGrid.y; j++) {
          for (int k = minGrid.z; k <= maxGrid.z; k++) {
              int neighborGridIdx1D = gridIndex3Dto1D(i, j, k, gridResolution);
              // - For each cell, read the start/end indices in the boid pointer array.
              int start = gridCellStartIndices[neighborGridIdx1D];
              int end = gridCellEndIndices[neighborGridIdx1D];

              // - Access each boid in the cell and compute velocity change from
              //   the boids rules, if this boid is within the neighborhood distance.
              for (int s = start; s < end; s++) {
                  int neighborArrayIdx = particleArrayIndices[s]; 
                  if (index == neighborArrayIdx) continue;
                  glm::vec3 thatBoidPos = pos[neighborArrayIdx];
                  glm::vec3 thatBoidVelocity = vel1[neighborArrayIdx];

                  float distance = glm::length(thatBoidPos - currPos);
                  if (distance < rule1Distance) {
                      perceivedCenter += thatBoidPos;
                      neighborCount1++;
                  }
                  if (distance < rule2Distance) {
                      c -= (thatBoidPos - currPos);
                  }
                  if (distance < rule3Distance) {
                      perceivedVelocity += thatBoidVelocity;
                      neighborCount3++;
...
__global__ void kernRearrangeVector(int N, glm::vec3 *og_pos, glm::vec3 *og_vel, glm::vec3 *new_pos, glm::vec3 *new_vel, int *particleArrayIndices) {
    int index = threadIdx.x + (blockIdx.x * blockDim.x);
    if (index >= N) {
        return;
    }
    new_vel[index] = og_vel[particleArrayIndices[index]];
    new_pos[index] = og_pos[particleArrayIndices[index]];
}
  ...
  for (int i = minGrid.x; i <= maxGrid.x; i++) {
      for (int j = minGrid.y; j <= maxGrid.y; j++) {
          for (int k = minGrid.z; k <= maxGrid.z; k++) {
              ...
              for (int s = start; s < end; s++) {
                  if (index == s) continue;
                  glm::vec3 thatBoidPos = pos[s]; //pos and vel1 should be shuffled! 
                  glm::vec3 thatBoidVelocity = vel1[s];
...
void Boids::stepSimulationScatteredGrid(float dt) {
  ...
  kernUpdateVelNeighborSearchScattered << <blocksPerGridCell, blockSize >> > (
